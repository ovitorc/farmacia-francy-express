...
function DesktopCategoryMenu({ categoria }: { categoria: Categoria }) {
  ...
  const atualizarPosicao = () => {
    ...
    const largura = Math.min(980, Math.max(320, window.innerWidth - 24));

    let left = 0;

    if (rect.left + left + largura > window.innerWidth - 12) {
      left = window.innerWidth - 12 - largura - rect.left;
    }
  ...
  return (
    <div ref={gatilhoRef} className="relative shrink-0" onMouseEnter={abrirMenu} onMouseLeave={fecharMenu}>
      <Link
        to="/categoria/$slug"
        params={{ slug: categoria.slug }}
      ...
      {menuAberto && (
        <div
          className="absolute top-full z-[9999] overflow-visible"
          style={{
            left: menuLeft,
        ...
        >
          <div className="w-full rounded-b-2xl border-x border-b border-border/80 bg-popover/98 p-4 text-popover-foreground shadow-[0_20px_60px_rgba(0,0,0,0.18)] backdrop-blur-xl">
              ...
              {categoria.subcategorias.map((subcategoria) => {
                ...
                return (
                  ...
                  >
                    ...
                    {temTerceiroNivel && estaAberta && (
                      <div className="mt-0.5 border-l border-border pl-2">
                        {subcategoria.subcategorias?.map((terceiro) => (
                          <Link
                            key={terceiro.slug}
                            to="/categoria/$slug"
                            ...
                            className="block break-words rounded-md px-2 py-0.5 text-xs leading-tight text-muted-foreground hover:bg-accent hover:text-foreground"
                          >
                            {terceiro.nome}
                          </Link>
                        ))}
