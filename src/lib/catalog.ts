...
export const ESTRUTURA_CATEGORIAS_LEGADA: Categoria[] = [
  {
...
const sub = (nome: string, slug: string, subcategorias?: Subcategory[]): Subcategory => ({
  nome,
  slug,
  ...(subcategorias && subcategorias.length > 0 ? { subcategorias } : {}),
});

const gruposMedicamentos = criarSubcategorias([
  ["De marca", "marca"],
  ["Genéricos", "genericos"],
  ["Similares", "similares"],
  ["MIPs", "mips"],
...
const terapeuticasMedicamentos = (tipo: string) =>
  gruposMedicamentos.map((item) =>
    sub(
      item.nome,
      item.slug,
      ESTRUTURA_CATEGORIAS_LEGADA.find((c) => c.slug === (tipo === "mips" ? "mips" : `medicamentos-${tipo}`))
        ?.subcategorias ?? [],
    ),
  );
...
export const ESTRUTURA_CATEGORIAS_SITE: Categoria[] = [
  ...
  {
    nome: "Higiene e Cuidados Pessoais",
...
function classificarProdutoNoSiteInterna(produto: Produto): {
  categoria: string;
  subcategoria: string;
} {
...
function mapearClassificacaoParaNovaEstrutura(classificacao: { categoria: string; subcategoria: string }) {
  ...
  if (
    c === "medicamentos-de-marca" ||
    c === "medicamentos-genericos" ||
    c === "medicamentos-similares" ||
    c === "mips"
  ) {
    const mapa: Record<string, string> = {
      "medicamentos-de-marca": "marca",
      "medicamentos-genericos": "genericos",
...
export function produtosDaCategoriaSite(produtos: Produto[], categoria: string, sub?: string, subsub?: string) {
  return produtos.filter((produto) => {
    const classificacao = classificarProdutoNoSite(produto);

    return (
      classificacao.categoria === categoria &&
      (!sub || classificacao.subcategoria === sub) &&
      (!subsub || classificacao.subsubcategoria === subsub)
    );
  });
}
